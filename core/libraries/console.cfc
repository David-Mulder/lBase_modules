<cfcomponent extends="system.library" lbase:scope="request">
	...
	<cffunction 
		...
		<cfif arrayFind(this.loggedVariables,getHashCode(arguments.var))>
			<cfreturn '"Already logged somewhere else in this object (not in order)"' />
		</cfif>
		...
		<!--- OBJECT --->
		<cfelseif IsValid("component",_data)>
			<cfset arKeys = StructKeyArray(_data) />
			<cfset componentRepresentation = {}>
			<cfloop from="1" to="#ArrayLen(arKeys)#" index="i">
				<cfif IsCustomFunction(_data[arKeys[i]])>
					<cfset componentRepresentation[arKeys[i] & "()"] = getMetaData(_data[arKeys[i]])>
				<cfelse>
					<cfset componentRepresentation[arKeys[i]] = _data[arKeys[i]]>
				</cfif>
			</cfloop>
			<cfreturn serializeToJson(componentRepresentation, arguments.serializeQueryByColumns, arguments.strictMapping) />	
